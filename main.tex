...
We again start with $E[C] \geq t * (1 - (1 - \rho)^{c})$.\\
$\geq t * (1 - e^{-\rho c})$\\
$= t * (1 - e^{-m/n *c})$\\
...
\end{lemma}
\begin{proof}
During this round, every party will receive at least $t/2$ bundles that may hold $m_e$, and at most $t/2 + 1$ parties do not have $m_e$ at the beginning of the round. The probability that at least one party does not have $m_e$ by the end of the round regardless is at most $(t/2 + 1) * (1 - \rho)^c$.\\
 $(t/2 + 1) * (1 - \rho)^c = (t/2 + 1) * (1 - m/n)^c$\\
 $< (t/2 + 1) * (1 - m/n)^{t/2} \leq (t/2 + 1) * e^{-\frac m n * \frac t 2}$.\\
 Then the probability that all parties have $m_e$ by the end of the round is at least $(t/2 + 1) * (1 - \rho)^c$.
\end{proof}
...
// all nodes\\
\Upon{receiving $<PROPOSE, M>$ from the broadcaster do}
{
	let $h := hash(M)$\\
	send $<ECHO, h>$ to all
}
\Upon{receiving $2t+1 <ECHO, h>$ messages and not having sent a $READY$ message do}
{
	send $<READY, h>$ to all
}
\Upon{receiving $t+1$ $<READY, h>$ messages and not having sent a $READY$ message do}
{
	send $<READY, h>$ to all\\
}
\Upon{receiving $2t+1 <READY, h>$ messages do}
{
	\If{received $<PROPOSE, M>$ and $h = hash(M)$}
	{
		$ADD(M)$
	}
	\Else
...
\Upon{receiving a bundle $<r', B>$ where $r' = r$}
{
	$ReceiveBundle(B)$
}
...
\Procedure{Gossip(local)}
{
	\For{each $p_i$}
	{
		send $<r, local>$ to $p_i$ with probability $\rho$. Otherwise send $<r, \bot>$ to $p_i$.
	}
}

\Procedure{ReceiveBundle(B)}
{
	\If{$B$ is not empty and $hash(B) = h$ and $local = \bot$}
	{
		$local \leftarrow B$
	}
