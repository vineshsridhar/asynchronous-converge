...
A message $m_e$ is \emph{eminent} if held by at least $n - 2t$ parties ($= t+1$ if $t$ is maximal). This distinction is interesting because, upon crossing this threshold, in every subsequent round, every party that does not have $m_e$ will receive at least 1 bundle that contains $m_e$. Furthermore, if $m_e$ is held by $n - 2t + c$ parties at the start of a round, every party that does not have $m_e$ will receive at least $c+1$ bundles that contain $m_e$ during that round. 

Note that $0 \leq c \leq t$ if $m_e$ is eminent. We want to show how fast the remaining $n - t - (n - 2t + c) = t - c$ (at most $t$) parties receive $m_e$. We can borrow the analysis from PBC's lemma 5 and 9~\cite{PBC}, except instead of propagating to $n$ parties, we only need to propagate to $t$ parties. 
...
\emph{Case 1}: \\
When $c \leq t/5$, we can apply the lower-tail Chernoff bound. Let $C$ be a random variable that tracks the size of $c$.\\
Then $\mu_C$ must be at least $t * (1 - (1 - \rho)^{c+1})$, as there are at most $t$ parties that must receive $m_e$, and each would receive $m_e$ from at least $c+1$ bundles.\\
$t * (1 - (1 - \rho)^{c+1}) \geq t * (1 - (1 - \rho)^{c+1})$\\
$=t * (1 - (1 - m/n))$\\
$=t * m/n$
...
We again start with $\mu_C \geq t * (1 - (1 - \rho)^{c+1})$.\\
$\geq t * (1 - e^{-\rho(c+1)})$\\
$= t * (1 - e^{-m/n (c+1)})$\\
$\geq 5c * (1 - e^{-m/n(c+1)})$ as $c \leq t/5$ and so $t \geq 5c$\\
$\geq 5c * (1 - e^{-m/n(t/5 +1)})$\\
$\geq 5c * (1 - e^{-m/n(t/5)})$\\
...
If we consider some party $p_{\cancel{m_e}}$ that does not hold $m_e$, the probability that $p_{\cancel{m_e}}$ does \emph{not} receive $m_e$ during the next round is at most $(1 - \rho)^{c+1}$ because at least $c+1$ bundles will be delivered to $p_{\cancel{m_e}}$ that contain $m_e$.\\
$(1 - \rho)^{c+1} < (1 - \rho)^{t/5+1}$ because $c > t/5$\\
$< (1 - \rho)^{t/5}$\\
$\leq e^{-\rho*t/5}$\\
